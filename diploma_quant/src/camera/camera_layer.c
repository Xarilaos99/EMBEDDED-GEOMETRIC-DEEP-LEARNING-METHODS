...
uint8_t           GrayScale [ 240  * 320  ] BSP_PLACE_IN_SECTION(".sdram") BSP_ALIGN_VARIABLE(8);




uint8_t correct_digit =(uint8_t)0;
...
void Yuv422_to_G(const void* inbuf, void* outbuf, uint16_t width, uint16_t height){
    uint32_t rows, columns;
    int32_t  y;
    uint8_t  *yuv_buf;
  ...
  //  SCB_EnableDCache();
    ...
    for (rows = 0; rows < height; rows++)
    {
        for (columns = 0; columns < width; columns++)
        {
            ...
            //if you want to rotate correct the image 
            // int newX = height - 1 - rows;
            // int dstOffset = (newY * height + newX) ;
...
bool_t camera_init(bool_t use_test_mode)
{
    ...
    if ((reg_val2 == 0x40) || (reg_val2 == 0x41) || (reg_val2 == 0x4C))
    {
        ...
        // R_CEU_CaptureStart(&g_ceu_ctrl, (uint8_t * const )&bsp_camera_out_buffer);
...
void bsp_camera_capture_image(void)
{
    ...
    // Wait for capture to finish
        ...
        //remember that yuv4222rgb565 and rot90 is for printing at the lcd the camera

        adjust_brightness(bsp_camera_out_buffer, bsp_camera_out_buffer, BSP_CAM_WIDTH, BSP_CAM_HEIGHT);

        Yuv422_to_G(bsp_camera_out_buffer, GrayScale, BSP_CAM_WIDTH, BSP_CAM_HEIGHT);
